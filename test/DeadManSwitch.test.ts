...
contract("DeadManSwitch", accounts => {
  ...
  it("should allow beneficiary account to claim the ether", async () => {
    ...
    const intialBalance = new BigNumber(await web3.eth.getBalance(beneficiary));
    ...
    await deadManSwitch.refund(zeroAddress, oneEther, {
      from: beneficiary
    });
    const finalBalance = new BigNumber(await web3.eth.getBalance(beneficiary));
    assert.equal(
  ...
  it("allows transfer of erc 20 token into the contract", async () => {
    ...
    assert.equal(
      (await erc20.balanceOf(owner)).toString(),
      totalSupply.toString()
    );
    await erc20.approve(deadManSwitch.address, totalSupply);
    await deadManSwitch.depositERC20(erc20.address, totalSupply);
    assert.equal(
      (await erc20.balanceOf(deadManSwitch.address)).toString(),
      totalSupply.toString()
    );
    await deadManSwitch.refund(erc20.address, totalSupply);
    assert.equal(
      (await erc20.balanceOf(owner)).toString(),
      totalSupply.toString()
    );
